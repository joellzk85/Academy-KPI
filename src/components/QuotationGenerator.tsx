  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      if (firestoreQuotes.length > 0) {
        ...
        // No quotations in Firestore yet, let's migrate the local ones!
        const localSaved = localStorage.getItem('next_quotations_lzk.joel@gmail.com');
        let parsed: Quotation[] = [];
        if (localSaved) {
          try {
        ...
        if (!parsed || parsed.length === 0) {
          parsed = [JSON.parse(JSON.stringify(defaultQuoteTemplate))];
        }
        if (parsed && parsed.length > 0) {
          for (const item of parsed) {
            const docId = item.id || `q_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
