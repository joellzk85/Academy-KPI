  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      if (firestoreOutlines.length > 0) {
        ...
        // No outlines in Firestore yet, let's migrate the local ones!
        const localSaved = localStorage.getItem('next_course_outlines_lzk.joel@gmail.com');
        ...
        if (!parsed || parsed.length === 0) {
          parsed = [JSON.parse(JSON.stringify(DEFAULT_OUTLINE_TEMPLATE))];
        }
        if (parsed && parsed.length > 0) {
          for (const item of parsed) {
            const docId = item.id || `co_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
            await setDoc(doc(db, 'course_outlines', docId), {
              ...item,
              id: docId
            });
          }
